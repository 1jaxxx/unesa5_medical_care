                    ...
                    <form action="{{ route('admin.resep.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <select name="id_obat" id="id_obat" class="form-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    ...
                                    @foreach ($obat as $item)
                                        <option value="{{ $item->id_obat }}" 
                                                data-stok="{{ $item->stok }}"
                                                {{ old('id_obat') == $item->id_obat ? 'selected' : '' }}
                            ...
                            <div>
                                ...
                                <select name="id_visit" id="id_visit" class="form-select mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    ...
                                    @foreach ($visits as $visit)
                                        <option value="{{ $visit->id_visit }}" {{ old('id_visit') == $visit->id_visit ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="date" name="tgl_diberikan" id="tgl_diberikan" value="{{ old('tgl_diberikan', date('Y-m-d')) }}" class="form-input mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                            </div>
                            <div>
