                    ...
                    <div class="mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <div class="flex flex-col md:flex-row justify-between gap-4">
                            ...
                            <div class="flex-1 space-y-4">
                                <form method="GET" action="{{ route('admin.visit.index') }}">
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    @php
                                        $statuses = ['all' => 'Semua', 'pending' => 'Pending', 'inprogress' => 'In Progress', 'completed' => 'Completed'];
                                    @endphp
                                    <div class="flex flex-wrap rounded-md -space-x-px">
                                        @foreach($statuses as $key => $label)
                                        <a href="{{ route('admin.visit.index', ['status' => $key, 'search' => $search]) }}" @class([
                                            'px-3 py-1.5 text-sm font-medium border',
                                            'rounded-l-md' => $loop->first,
                                            'rounded-r-md' => $loop->last,
                                            'bg-blue-600 text-white border-blue-600 z-10' => (request('status', 'all') === $key),
                                            'bg-white text-gray-600 hover:bg-gray-50 border-gray-300' => (request('status', 'all') !== $key),
                                        ])>
                                            {{ $label }}
                                        </a>
                                        @endforeach
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-end gap-3 mt-2 md:mt-0">
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('admin.visit.export.excel', request()->all()) }}"
                                     ...
                                     <a href="{{ route('admin.visit.export.pdf', request()->all()) }}"
                     ...
                     @php
                        ...
                        function sortUrl($field)
                        {
                            ...
                            return route('admin.visit.index', array_filter($params));
                        }
                    @endphp
                                ...
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                        <a href="{{ sortUrl('tgl_kunjungan') }}" class="flex items-center gap-1">
                                            Tgl Kunjungan <span class="text-gray-400">{{ sortIcon('tgl_kunjungan') }}</span>
                                        </a>
                                    </th>
                                    <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                        Pasien
                                        ...
                                        <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <p class="mt-1 text-sm text-gray-500">
                                                    Tidak ada data kunjungan yang cocok dengan pencarian atau filter Anda.
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
