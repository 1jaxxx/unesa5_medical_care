...
<script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const prodiField = document.querySelector('.prodi-field'); // Elemen baru
            
            // Ambil juga input select 'prodi' untuk menonaktifkannya
            const prodiSelect = document.getElementById('id_prodi');

            // Buat fungsi untuk mengatur field berdasarkan tipe pasien
            function updateFormFields() {
                const selectedValue = typePasienSelect.value;

                // 1. Sembunyikan semua field dinamis terlebih dahulu
                nimField.classList.add('hidden');
                nidnField.classList.add('hidden');
                bagianField.classList.add('hidden');
                prodiField.classList.add('hidden'); // Sembunyikan prodi
                
                // 2. Nonaktifkan input prodi (penting agar datanya tidak terkirim)
                prodiSelect.disabled = true;

                // 3. Tampilkan field yang sesuai berdasarkan pilihan
                if (selectedValue === 'mahasiswa') {
                    nimField.classList.remove('hidden');
                    prodiField.classList.remove('hidden'); // Tampilkan prodi
                    prodiSelect.disabled = false; // Aktifkan lagi input prodi
            ...
            // 4. Tambahkan event listener untuk memanggil fungsi saat pilihan berubah
            typePasienSelect.addEventListener('change', updateFormFields);

            // 5. Panggil fungsi sekali saat halaman dimuat
            // Ini untuk menangani nilai 'old()' jika terjadi error validasi
            updateFormFields();
        });
    </script>
</x-admin-layout>
